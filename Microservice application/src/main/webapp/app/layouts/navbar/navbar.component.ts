...
export class NavbarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.accountService.getAuthenticationState().subscribe(account => {
      ...
      import('microfe/entity-navbar-items').then(
        async ({ EntityNavbarItems: MicrofeEntityNavbarItems }) => {
          this.microfeEntityNavbarItems = MicrofeEntityNavbarItems;
          const { LazyTranslationModule } = await import('microfe/translation-module');
          this.loadModule(LazyTranslationModule as Type<any>);
        },
        error => {
          // eslint-disable-next-line no-console
          console.log('Error loading microfe entities', error);
        }
      );
    });
  }
